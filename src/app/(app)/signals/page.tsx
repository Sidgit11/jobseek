...
import { Loader2, Copy, Check, ExternalLink, Zap, Radio, ChevronDown, ChevronUp, MapPin, Users, Building2, FileText, Briefcase, Search, ArrowUpDown, Chrome, Pause, Play, RefreshCw } from 'lucide-react'
...
function ExtensionStatusBar() {
  ...
  if (!ext.found) {
    return (
      <div
        className="mx-6 mt-3 flex items-center justify-between rounded-xl px-4 py-2.5"
        style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)' }}
      >
        <div className="flex items-center gap-2.5">
          <div className="h-2 w-2 rounded-full" style={{ background: '#EF4444' }} />
          <span className="text-xs font-medium" style={{ color: C.text }}>Extension not detected</span>
          <span className="text-[11px]" style={{ color: C.muted }}>Install it to start capturing signals</span>
        </div>
        <a
          ...
          style={{ background: C.accent, color: '#1A2E05' }}
        >
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-medium" style={{ color: C.text }}>Extension paused</span>
          <span className="text-[11px]" style={{ color: C.muted }}>v{ext.version} · Last scan: {formatLastScan(ext.lastScanTime)}</span>
        </div>
        <button
          ...
          style={{ background: C.accentGlow, color: C.accentSoft, border: '1px solid var(--color-lime-border)' }}
        >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-xs font-medium" style={{ color: C.text }}>Extension connected</span>
        <span className="text-[11px]" style={{ color: C.muted }}>v{ext.version} · Last scan: {formatLastScan(ext.lastScanTime)}</span>
      </div>
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          disabled={acting}
          className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-medium transition-all hover:brightness-110 cursor-pointer"
          style={{ background: 'var(--color-surface)', color: C.muted, border: `1px solid ${C.border}` }}
        >
          <Pause size={11} /> Pause
        </button>
      </div>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-6" style={{ background: C.bg }}>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="mt-6 flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-semibold transition-all hover:brightness-110"
        style={{ background: C.accent, color: '#1A2E05' }}
      >
