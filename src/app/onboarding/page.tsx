...
const EXTENSION_STORE_URL = 'https://chromewebstore.google.com/detail/jobseek/YOUR_EXTENSION_ID'
const KNOWN_EXTENSION_IDS: string[] = [
  // Add your Chrome Web Store extension ID here after publishing
]
...
async function pingExtension(extId: string): Promise<{ success: boolean; version?: string; deviceToken?: string }> {
  ...
  return new Promise((resolve) => {
    ...
    try {
      chromeApi.runtime!.sendMessage!(extId, { action: 'PING' }, (resp: unknown) => {
        clearTimeout(timer)
        const r = resp as { success?: boolean; version?: string; deviceToken?: string } | undefined
        resolve(r?.success ? { success: true, version: r.version, deviceToken: r.deviceToken } : { success: false })
      })
    } catch { clearTimeout(timer); resolve({ success: false }) }
  })
}

async function detectExtension(): Promise<{ found: boolean; extId?: string; version?: string; deviceToken?: string }> {
  const chromeApi = (window as unknown as { chrome?: { runtime?: { sendMessage?: unknown } } }).chrome
  ...
  if (stored) {
    ...
    if (r.success) return { found: true, extId: stored, version: r.version, deviceToken: r.deviceToken }
  }
  for (const id of KNOWN_EXTENSION_IDS) {
    if (id === stored) continue
    const r = await pingExtension(id)
    if (r.success) { localStorage.setItem('jobseek_extension_id', id); return { found: true, extId: id, version: r.version, deviceToken: r.deviceToken } }
  }
  return { found: false }
}
